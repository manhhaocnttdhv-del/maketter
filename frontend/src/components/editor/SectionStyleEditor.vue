...
import { ref } from 'vue'
import { CopyCheck, Globe, ImagePlus, Loader2, RotateCcw } from '@lucide/vue'
...
const handleImageUpload = async (event: Event) => {
  ...
  if (file.size > 10 * 1024 * 1024) {
    emit('upload-error', 'Ảnh nền lớn hơn 10 MB. Hãy nén ảnh trước khi tải lên.')
    return
  }
  ...
  try {
    ...
    emit('upload-error', (err instanceof Error && err.message) ? err.message : 'Không thể tải ảnh nền lên máy chủ.')
  } finally {
